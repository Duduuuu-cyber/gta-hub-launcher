...
process.env.DIST = require$$1$1.join(__dirname$1, "../dist_build");
process.env.PUBLIC = require$$1$3.app.isPackaged ? process.env.DIST : require$$1$1.join(__dirname$1, "../public");
...
function createWindow() {
  ...
  if (process.env.VITE_DEV_SERVER_URL) {
    ...
    win.loadFile(require$$1$1.join(process.env.DIST, "index.html"));
  }
}
...
require$$1$3.app.whenReady().then(() => {
  ...
  require$$1$3.ipcMain.on("maximize-window", () => {
  ...
  require$$1$3.ipcMain.on("close-window", () => {
    win == null ? void 0 : win.close();
  });
  require$$1$3.ipcMain.on("get-server-info", async (event) => {
  ...
  require$$1$3.ipcMain.handle("select-game-directory", async () => {
  ...
  require$$1$3.ipcMain.handle("select-directory", async (event, title) => {
    const result = await require$$1$3.dialog.showOpenDialog(win, {
      properties: ["openDirectory"],
  ...
  require$$1$3.ipcMain.on("launch-game", async (event, gamePath) => {
    if (!gamePath) return;
    const dns = await import("dns/promises");
    ...
    try {
      ...
      const sampExe = require$$1$1.join(gamePath, "samp.exe");
  ...
  require$$1$3.ipcMain.handle("get-registry-value", (event, key, valueName) => {
    return new Promise((resolve) => {
      require$$1$4.exec(`reg query "${key}" /v "${valueName}"`, (err, stdout) => {
        if (err) {
          resolve("");
          return;
        }
  ...
  require$$1$3.ipcMain.handle("set-registry-value", (event, key, valueName, data) => {
    return new Promise((resolve) => {
      require$$1$4.exec(`reg add "${key}" /v "${valueName}" /t REG_SZ /d "${data}" /f`, (err) => {
        resolve(!err);
      });
    });
  });
  require$$1$3.ipcMain.on("open-external", (event, url) => {
