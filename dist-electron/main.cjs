...
const __filename$1 = require$$2$1.fileURLToPath(typeof document === "undefined" ? require("url").pathToFileURL(__filename).href : _documentCurrentScript && _documentCurrentScript.tagName.toUpperCase() === "SCRIPT" && _documentCurrentScript.src || new URL("main.cjs", document.baseURI).href);
const __dirname$1 = require$$1.dirname(__filename$1);
const { autoUpdater } = pkg;
process.env.DIST = require$$1.join(__dirname$1, "../dist_build");
process.env.PUBLIC = require$$1$2.app.isPackaged ? process.env.DIST : require$$1.join(__dirname$1, "../public");
...
function createWindow() {
  win = new require$$1$2.BrowserWindow({
    ...
    webPreferences: {
      ...
      // For simple IPC demo, can be tightened later
    },
  ...
  if (process.env.VITE_DEV_SERVER_URL) {
    ...
    win.loadFile(require$$1.join(process.env.DIST, "index.html"));
  }
}
...
require$$1$2.app.whenReady().then(() => {
  ...
  require$$1$2.ipcMain.on("get-server-info", async (event) => {
  ...
  require$$1$2.ipcMain.handle("select-game-directory", async () => {
  ...
  require$$1$2.ipcMain.handle("select-directory", async (event, title) => {
    const result = await require$$1$2.dialog.showOpenDialog(win, {
      properties: ["openDirectory"],
  ...
  require$$1$2.ipcMain.on("launch-game", async (event, gamePath) => {
    if (!gamePath) return;
    const dns = await import("dns/promises");
    ...
    try {
      ...
      const sampExe = require$$1.join(gamePath, "samp.exe");
  ...
  require$$1$2.ipcMain.handle("get-registry-value", (event, key, valueName) => {
    return new Promise((resolve) => {
      require$$1$3.exec(`reg query "${key}" /v "${valueName}"`, (err, stdout) => {
        if (err) {
          resolve("");
          return;
        }
  ...
  require$$1$2.ipcMain.handle("set-registry-value", (event, key, valueName, data) => {
    return new Promise((resolve) => {
      require$$1$3.exec(`reg add "${key}" /v "${valueName}" /t REG_SZ /d "${data}" /f`, (err) => {
        resolve(!err);
      });
    });
  });
  require$$1$2.ipcMain.on("open-external", (event, url) => {
  ...
  require$$1$2.ipcMain.on("download-game-start", (event, url, targetPath) => {
    const file2 = fs$j.createWriteStream(targetPath);
    https.get(url, (response) => {
      ...
      fs$j.unlink(targetPath, () => {
      });
      event.sender.send("download-error", err.message);
    });
  });
  require$$1$2.ipcMain.handle("extract-game", async (event, zipPath, targetDir) => {
    try {
      const zip = new AdmZip(zipPath);
      zip.extractAllTo(targetDir, true);
      fs$j.unlinkSync(zipPath);
      return { success: true };
    } catch (e) {
      console.error(e);
  ...
  require$$1$2.ipcMain.handle("get-app-path", () => {
    return require$$1$2.app.isPackaged ? require$$1.dirname(require$$1$2.app.getPath("exe")) : require$$1$2.app.getAppPath();
  });
});
